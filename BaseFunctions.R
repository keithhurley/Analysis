...
base.create.aggregate.variables <- function(mydata, myYears) {
  ...
  data(zipcode)
  myUrbanZips <- zipcode %>%
    filter(city %in% myUrbanCities) %>%
    pull(zip)

  sf_urbanZips <- sf_urbanZips %>%
    mutate(zc = ZCTA5CE10) %>%
    filter(zc %in% myUrbanZips)

  #save(myUrbanZips, sf_urbanZips, file="..//..//Data//sf_urbanZips.rData")
  ...
  #save(myOmahaZips, sf_OmahaZips, file="..//..//Data//sf_OmahaZips.rData")

  mydata <- mydata %>%
    mutate(urban = ifelse(E8 %in% sf_urbanZips$ZCTA5CE10, TRUE, FALSE))
...
base.data.createFactors <- function(myDataframe) {
  myFactors <- base.loaddata.factorlevels() %>%
    filter(Value < 900)
  f <- unique(myFactors[, c("Field", "Value", "Label")])
