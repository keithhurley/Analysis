...
caption.listwiseDev <-
  "Computed on development-half respondents who answered all 13 items (listwise deletion). The test half is not used until Chapter 8. A pairwise-deletion sensitivity check is reported in Chapter 1."
caption.loadings <-
...
# Universe ----------------------------------------------------------------
# Same rule in every wave: the year, a non-missing preferred species, and a
...
#
# `years` may be a vector, which is how the pooled two-wave sample is built.
BuildD4Universe <- function(mydata, years) {
  mydata %>%
    filter(
      surveyYear %in% years,
      !is.na(B1),
...
# A random split of the pooled sample, stratified by survey year so neither
# half is accidentally wave-heavy. Assigns every respondent, including those
# with incomplete item batteries, so the split is a property of the sample
# rather than of item response.
SplitSample <- function(mydata, prop = 0.5, seed = d4.seed) {
  set.seed(seed)
  ...
  mydata %>%
    ...
    mutate(
      .rank = sample(n()),
      Half = if_else(.rank <= round(n() * prop), "Development", "Test")
    ) %>%
    ungroup() %>%
    select(-.rank)
}

SplitTable <- function(splitData) {
  splitData %>%
    mutate(
      Complete = complete.cases(ItemMatrix(splitData)),
      Wave = as.character(surveyYear)
    ) %>%
    group_by(Wave, Half) %>%
    summarise(
      Respondents = n(),
    ...
    pivot_wider(
      names_from = Half,
      values_from = c(Respondents, CompleteN)
    ) %>%
    transmute(
      Wave,
      `Development respondents` = formatC(
        Respondents_Development,
        format = "d",
        big.mark = ","
      ),
      `Development complete` = formatC(
        CompleteN_Development,
        format = "d",
        big.mark = ","
      ),
      `Test respondents` = formatC(
        Respondents_Test,
        format = "d",
        big.mark = ","
      ),
      `Test complete` = formatC(CompleteN_Test, format = "d", big.mark = ",")
    )
}

# Item means by wave. Pooling the two waves assumes the items behave similarly
# in each; this table is what makes that assumption inspectable rather than
# implicit. Descriptive only -- no test is reported, because 13 of them would
# invite reading noise as signal.
WaveItemTable <- function(mydata, itemText, items = d4.items) {
  mydata %>%
    select(surveyYear, all_of(items)) %>%
    pivot_longer(-surveyYear, names_to = "Item", values_to = "Response") %>%
    filter(!is.na(Response)) %>%
    mutate(v = as.numeric(Response)) %>%
    group_by(Item, Wave = as.character(surveyYear)) %>%
    summarise(N = n(), Mean = mean(v), SD = sd(v), .groups = "drop") %>%
    pivot_wider(
      names_from = Wave,
    ...
    transmute(
      Item,
      Statement = Text,
      `2018 mean` = paste0(Fmt2(Mean_2018), " (", Fmt2(SD_2018), ")"),
      `2018 N` = formatC(N_2018, format = "d", big.mark = ","),
      `2025 mean` = paste0(Fmt2(Mean_2025), " (", Fmt2(SD_2025), ")"),
      `2025 N` = formatC(N_2025, format = "d", big.mark = ","),
      Difference = Fmt2(Mean_2025 - Mean_2018)
    )
}
...
SensitivityTable <- function(Rlist, Rpair, nList, nPair, k) {
  ...
  # Factors are order- and sign-indeterminate, so each factor of the first
  # solution is matched to its best counterpart in the second on absolute
  # congruence. Comparing column 1 to column 1 would report a permutation as
  # a disagreement.
  cong <- apply(
    abs(psych::factor.congruence(
      AlignLoadings(fList),
      AlignLoadings(fPair)
    )),
    1,
...
# Chapter 8: CFA ----------------------------------------------------------
# Latent variable tag for a scale name. Named scales use the fixed table;
# anything else (the rule-derived four-factor alternative, whose factors have
# no content name) falls back to a sanitized abbreviation.
FactorTag <- function(scaleName) {
  tag <- d4.factor.tags[scaleName]
  if (!is.na(tag)) {
    return(unname(tag))
  }
  str_replace_all(toupper(scaleName), "[^A-Z0-9]", "")
}
...
CFAFitTable <- function(fitList) {
  map_dfr(names(fitList), function(nm) {
    ...
    tibble(
      Model = nm,
      # Models differ in how many items they include, so N is reported per
      # model rather than once for the table.
      N = formatC(
        lavaan::lavInspect(f, "nobs"),
        format = "d",
        big.mark = ","
      ),
      Items = length(lavaan::lavNames(f, "ov")),
...
# Fit the retained model separately within each wave of a sample. The pooled
# split tests replication across respondents but not across time; this is what
# recovers the time dimension the earlier wave-based design had built in.
WaveFitTable <- function(model, splitData, items) {
  waves <- sort(unique(splitData$surveyYear))

  map_dfr(waves, function(w) {
    dw <- splitData %>%
      filter(surveyYear == w) %>%
      ItemMatrix(items = items) %>%
    ...
    tibble(
      Wave = as.character(w),
      N = formatC(nrow(dw), format = "d", big.mark = ","),
      `Scaled chi-square` = Fmt1(m[["chisq.scaled"]]),
      df = formatC(m[["df.scaled"]], format = "d"),
      CFI = Fmt3(m[["cfi.scaled"]]),
      TLI = Fmt3(m[["tli.scaled"]]),
...
# Loading agreement between two fitted models, as Tucker congruence per factor.
WaveCongruenceTable <- function(model, splitData, items) {
  waves <- sort(unique(splitData$surveyYear))

  loadings <- lapply(waves, function(w) {
    dw <- splitData %>%
      filter(surveyYear == w) %>%
      ItemMatrix(items = items) %>%
      filter(complete.cases(.))

    est <- lavaan::standardizedSolution(FitCFA(model, dw)) %>%
      filter(op == "=~")

    est %>%
      select(lhs, rhs, est.std) %>%
      pivot_wider(names_from = lhs, values_from = est.std, values_fill = 0) %>%
      arrange(match(rhs, items))
  })

  m1 <- as.matrix(loadings[[1]][, -1])
  m2 <- as.matrix(loadings[[2]][, -1])

  tibble(
    Factor = colnames(m1),
    `Tucker congruence` = Fmt3(diag(psych::factor.congruence(m1, m2)))
  )
}
