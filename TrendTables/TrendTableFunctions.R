...
# Restore the original (2018-report) weighted-median display, overriding the
# quartile version in BaseFunctions_2025_UPDATED.R for this report only. The
# published convention shows: Value = weighted median (50th pct), CIlower =
# median (50th pct), CIupper = 95th pct, Number = raw respondent count.
# weighted.quantile() and postWeight come from the sourced base functions/data.
base.summary.medians <- function(mydata, myQuestion, myGroupVar = NA) {
  ...
  qData %>%
    ...
    summarise(
      medianValue = as.numeric(weighted.quantile(
        Answer,
        postWeight,
        probs = 0.5
      )),
      numValue = sum(!is.na(Answer)),
      CIupper = as.numeric(weighted.quantile(Answer, postWeight, probs = 0.95)),
      CIlower = as.numeric(weighted.quantile(Answer, postWeight, probs = 0.5)),
      .groups = "drop"
    ) %>%
...
CreateFt <- function(
  ...
  if (formatStyle == "perc") {
    myFlex <- myFlex %>%
      colformat_num(
        col_keys = c("2002_Value", "2012_Value", "2018_Value", "2025_Value"),
        big.mark = ",",
        digits = roundDigits,
        na_str = ""
      ) %>%
      colformat_num(
        col_keys = c("2002_CI", "2012_CI", "2018_CI", "2025_CI"),
        big.mark = ",",
        digits = roundDigits + 1,
        na_str = ""
      ) %>%
      colformat_num(
        col_keys = c(
...
CreatePercPlot <- function(
  mydata,
  freeScale = TRUE,
  numCols = 3,
  label_x = "Percent Of Respondants"
) {
  #munge data
  op <- mydata %>%
  ...
  op <- ggplot(op) +
    ...
    geom_errorbar(
      aes(x = Year, ymin = Value - CI, ymax = Value + CI),
      width = 0.5
    ) +
  ...
  op <- op +
    ...
    theme(
      ...
      strip.text = element_text(
        hjust = 1,
        face = "bold",
        color = viridis::viridis(3, option = "D")[1]
      )
    )
...
CreateMeanPlot <- function(
  mydata,
  label_x = "Mean Of Responses",
  freeScale = TRUE,
  numCols = 3
) {
  #munge data
...
CreateMedianPlot <- function(
  mydata,
  label_x = "Median Of Responses",
  freeScale = TRUE,
  numCols = 3
) {
  #munge data
...
CreateLikert_Grouped <- function(
  myData,
  myQuestions,
  myQuestionList,
  myTitle = ""
) {
  myLabels <- myQuestionList %>%
  ...
  myPlot <- plot(
    likert(op %>% select(-surveyYear), grouping = op$surveyYear),
    ordered = FALSE
  ) +
