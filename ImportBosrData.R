...
base.getBosrData <- function() {
  ...
  bosrFile <- "..\\..\\Data\\Anglers_final.sav"
  ...
  new_bData[, c(18:98, 101:270)] <- sapply(new_bData[, c(18:98, 101:270)], function(x) ifelse(is.na(x),-1, x))
  
  #convert to basic numeric columns
  new_bData[, c(18:98, 101:270)] <- sapply(new_bData[, c(18:98, 101:270)], function(x) as.numeric(x))
  
  #label venue
  new_bData <- new_bData %>%  mutate(venue = ifelse(DistributionChannel == "email", "email", "mail"))
  ...
  new_bData <- new_bData %>% select(18:270, venue, surveyYear, 
                                    licenseUID=ID, 
                                    OwnerCustomerUID=UID) #UID)
  
  #create combined B2 field
  newCol<-new_bData %>% select(contains("B2")) %>% select(contains("_lrp")) %>% names() %>% sub("_lrp", "", .)  
  lrpCol<-new_bData %>% select(contains("B2")) %>% select(contains("_lrp")) %>% names() 
  rscCol<-new_bData %>% select(contains("B2")) %>% select(contains("_rsc")) %>% names() 
  
  foreach(i = 1:length(newCol)) %do% {
    new_bData[,newCol[i]]<-ifelse(new_bData[,lrpCol[i]] + new_bData[,rscCol[i]] > 0, 1, 0)
  }
...
base.AddAgeGroupFor2018<-function(myD) {
  ...
  d<-myD %>%
    ...
    mutate(age_group=ifelse(is.na(E3) | E3==-1, as.numeric(Age_cat), as.character(E3))) 
